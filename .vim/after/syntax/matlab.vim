...
syn keyword matlabConditional		case else elseif end otherwise break continue
"syn keyword matlabConditional		switch case else elseif end if otherwise break continue
"syn keyword matlabRepeat		do for while
syn keyword matlabStorageClass		events persistent global
syn keyword matlabExceptions		catch rethrow throw
...
syn match structFieldNames "\w\.."
" String
syn region matlabCharArray		start=+'+ end=+'+	oneline
...
"Annotating Plots
syn keyword matlabFunc annotation clabel datacursormode datetick gtext legend  line rectangle texlabel title xlabel ylabel zlabel xlim ylim zlim view
...
"Graph Plots
syn keyword matlabFunc highlight
...
""""""""""""""""""""""""""
""if exists("b:current_syntax") ""let  g:dgianni = 'hoho'
""    syn region matlabFold start="\<for\>" nd="\<end\>" transparent fold
""
""endif
syn region matlabCodeFolds
            \ matchgroup=matlabRepeat
            \ start="\<for\>\|\<while\>\|\<try\>\|\<function\>\|\<if\>\|\<switch\>\|\<parfor\>\|\<spmd\>\|\<classdef\>\|\<properties\>\|\<methods\>"
            \ end="^\s*\zs\<end\>\ze\(\s*\(#.*\)*$\)"
            \ transparent
            \ fold
""
""syn region matlabBlockComment   start=+#{+ end=+#}+ fold contains=matlabTodo,matlabTab 

""syn region matlabCommentFold start="^\s*\([^#\s].*\)*\n\zs\s*#.*\ze\n\s*#.*$" end="\s*#.*\n\s*#.*\zs\n\ze\s*\([^#\s].*\)*$" transparent fold
"

" Define a region for folding consecutive MATLAB comments starting with %
""syntax region matlabCommentFold start=/^\s*%.*$/ end=/^\s*[^%\s].*$/
""syn match matlabCommentFoldd "^\s*\([^%\s]\)*\n\zs\s*%.*\n\s*%.*\(\n\s*%.*\)*$" fold

"
""syntax region matlabCommentFold start="^\s*\([^\sa].*\)*\n\s*\zsa\ze.*\n\s*a.*$" end="^\s*a.*\n\s*\zsa\ze.*\n\s*\([^\sa]+.*\)*$" fold  keepend extend
"
""syntax region ml start="^\s*\([^%\s].*\n\)*\zs\s*%.*\n\ze\s*%.*$" end="^\s*.*\n\ze\s*\([^%\s].*\)*$" fold 
"
"if exists("loaded_matchit")
"	let s:conditionalEnd = '^\s*\<end\>'
" let b:match_words=
"   \ '\<\%(if\|parfor\|switch\|for\|while\)\>:\<\%(elseif\|case\|break\|continue\|else\|otherwise\)\>:'.s:conditionalEnd.','.
